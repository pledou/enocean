...
class EEP(object):
    ...
    def _get_rangeitem(source, raw_value):
        for rangeitem in source.find_all("rangeitem"):
            if raw_value in range(
                int(rangeitem.get("start", -1)), int(rangeitem.get("end", -1)) + 1
            ):
                return rangeitem
    ...
    def _set_value(self, target, value, bitarray):
        ...
        scl_min = float(scl.find("min").text)
        scl_max = float(scl.find("max").text)
        raw_value = (value - scl_min) * (rng_max - rng_min) / (
            scl_max - scl_min
        ) + rng_min
    ...
    def _set_enum(self, target, value, bitarray):
        ...
        if isinstance(value, int):
            ...
            if target.find("item", {"value": value}) or self._get_rangeitem(
                target, value
            ):
            ...
            if value_item is None:
                raise ValueError(
                    'Enum description for value "%s" not found in EEP.' % (value)
                )
    ...
    def find_profile(
        self, bitarray, eep_rorg, rorg_func, rorg_type, direction=None, command=None
    ):
        ...
        if rorg_func not in self.telegrams[eep_rorg].keys():
            self.logger.warn(
                "Cannot find rorg %s func %s in EEP!", hex(eep_rorg), hex(rorg_func)
            )
            return None
    ...
    def set_values(self, profile, data, status, properties):
        ...
        for shortcut, value in properties.items():
            ...
            if not target:
                ...
                self.logger.warning(
                    "Cannot find data description for shortcut %s", shortcut
                )
                continue
